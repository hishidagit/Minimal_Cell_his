...
class MyOwnSolver(lm.GillespieDSolver):
    ...
    def hookSimulation(self, time):
        ...
        # We are at a CME-ODE communication timestep
            ...
            # Only compile once - model structure doesn't change between timesteps
            if self.cached_solver is None:
            ...
            if (int(time) % 10) == 0 and time > 0:
                print(
                    f'Progress: minute {self.global_minute}, time {int(time)}/60 seconds')
            ...
            if time >= (self.resTime - self.delt):
                print(
                    f"Saving flux data for minute {self.global_minute} at time {time}")
                ...
                for indx, rxn in enumerate(model.getRxnList()):
                    fluxList_start.append(
                        (rxn.getID(), currentFluxes_start[indx]))

                fluxDF_start = pd.DataFrame(fluxList_start)
                fluxFileName_start = '../simulations/fluxes/' + \
                    'rep-' + self.procID + '-fluxDF-start.csv'
                ...
                fluxFileName_final = '../simulations/fluxes/' + \
                    'rep-' + self.procID + '-fluxDF_final.csv'
                fluxDF_end.to_csv(fluxFileName_final, header=False, mode='a')

                # Also save to a per-minute flux file for compatibility
                fluxFileName_minute = '../simulations/fluxes/' + \
                    'rep-' + self.procID + '-fluxDF.csv'
