...
for sTime in np.arange(1, runTime+1, 1):
    ...
    def getFn(iteration):
        if (int(iteration) <= 1):
            oldFile = '../simulations/out-' + str(procid) + '.lm'
        else:
            print(iteration)
            itVal = int(iteration)-1
            # './simulations/batch1/rep-' + str(procid) + '.csv' #+ '/'+ str(int(iteration)-1) +'min-simDF_parts_end.csv'
            oldFile = '../simulations/out-' + \
                str(procid) + 'rs' + str(itVal) + '.lm'
        return oldFile
    ...
    def setInitVals(oldFile, sim):
        ...
        for spec in sL:
            ...
            # print("ADDING")
            if (oldFile == '../simulations/out-' + str(procid) + '.lm'):
    ...
    def setSpecVals(oldFile, sim, iteration, sL):
        ...
        try:
            ...
            # ,names=sL)#,skiprows=mySkip)
            rxnDFMin = pd.read_csv(oldFile, header=mySkip)
            specDict = {}
    ...
    proteomPD = pd.read_excel(
        "../model_data/proteomics.xlsx", sheet_name="Proteomics", skiprows=[0])
    ...
    def getPtnCount(newMetID, jcvi2ID, TrscBool=False):
        ...
        try:
            if jcvi2ID.startswith("JCVIman_"):
                aoeID = manGPRPD.loc[manGPRPD.MM == jcvi2ID.replace(
                    "JCVIman_", ""), "AOE"].values[0]
            else:
    ...
    #
            ...
            if (TrscBool == False):
                ...
                ptnCount = max(defaultPtnCount, round(
                    proteomPD.loc[proteomPD.Protein == aoeID].iloc[0, 21]))
    ...
    def getSequences(jcvi3AID):
        ...
        try:
            rnasequence = genomePtnLocDict[jcvi3AID].extract(
                genome3A.seq).transcribe()
            ...
            aasequence = genomePtnLocDict[jcvi3AID].extract(
                genome3A.seq).transcribe().translate(table=4)
    ...
    def getRNAsequences(jcvi3AID):
        # returns genomic and protein sequences
        try:
            rnasequence = genomeRnaLocDict[jcvi3AID].extract(
                genome3A.seq).transcribe()
    ...
    baseMap = OrderedDict(
        {"A": "M_atp_c", "U": "M_utp_c", "G": "M_gtp_c", "C": "M_ctp_c"})
    ...
    baseMap = OrderedDict(
        {"A": ATPconc, "U": UTPconc, "G": GTPconc, "C": CTPconc})
    ...
    def TranscriptRate(rnaMetID, ptnMetID, rnasequence, jcvi2ID):
        ...
        if (set(rnasequence) - set(baseMap.keys())):
            raise Exception("Unknown base(s) in RNA sequence {}".format(
                set(rnasequence) - set(baseMap.keys())))
        ...
        NMonoSum = NMono_A*rnaPolKd/ATPconc + NMono_C*rnaPolKd / \
            CTPconc + NMono_U*rnaPolKd/UTPconc + NMono_G*rnaPolKd/GTPconc

        k_transcription = kcat_mod / \
            ((1+rnaPolK0/RnaPconc)*(rnaPolKd**2) /
             (CMono1*CMono2) + NMonoSum + n_tot - 1)
    ...
    def TranslocRate(aasequence):
        ...
        if (set(aasequence) - set(aaMap.keys())):
            raise Exception("Unknown residue(s) in Protein sequence {}".format(
                set(aasequence) - set(aaMap.keys())))
    ...
    def addPtn(jcvi3AID):
        ...
        try:
            jcvi2ID = annotatPD.loc[annotatPD.iloc[:, 5]
                                    == mmcode].iloc[0, 13].strip()
        except:
    ...
    #             ptnDegProd.append('ATP_ptndeg')
            ...
            AaUsed = [['A', NMono_A], ['R', NMono_R], ['N', NMono_N], ['D', NMono_D], ['C', NMono_C],
                      ['E', NMono_E], ['Q', NMono_Q], ['H', NMono_H], [
                          'I', NMono_I], ['L', NMono_L],
                      ['K', NMono_K], ['M', NMono_M], ['P', NMono_P], [
                          'S', NMono_S], ['T', NMono_T],
                      ['W', NMono_W], ['Y', NMono_Y], ['G', NMono_G], [
                          'F', NMono_F], ['V', NMono_V],
                      ['FM', NMono_FM]]
            ...
            sim.addReaction(geneMetID, tuple(TrscProd), TranscriptRate(
                rnaMetID, ptnMetID, rnasequence, jcvi2ID))
            sim.addReaction(rnaMetID, tuple(mRNAdegProd),
                            DegradationRate(rnaMetID, rnasequence))
            sim.addReaction(rnaMetID, tuple(TranslatProd), TranslatRate(
                rnaMetID, ptnMetID, rnasequence, aasequence, specDict))
    ...
    def addNamedPtn(newMetID, mmcode, transcribe, ptnFrac):
        ...
        #         ptnDegProd.append('ATP_ptndeg')
            ...
            sim.addReaction(geneMetID, tuple(TrscProd), TranscriptRate(
                rnaMetID, newMetID, rnasequence, jcvi2ID))
            sim.addReaction(rnaMetID, tuple(mRNAdegProd),
                            DegradationRate(rnaMetID, rnasequence))
            sim.addReaction(rnaMetID, tuple(TranslatProd), TranslatRate(
                rnaMetID, newMetID, rnasequence, aasequence, specDict))
    ...
    def addMembranePtn(newMetID, mmcode, transcribe, ptnFrac):
        ...
        if transcribe:
            ...
            sim.addReaction(rnaMetID, tuple(TranslatProd), TranslatRate(
                rnaMetID, newMetID, rnasequence, aasequence, specDict))
            sim.addReaction(newMetID_cyto, tuple(
                TranslocProd), TranslocRate(aasequence))
    ...
    def addRiboPtn(newMetID, mmcode):
        ...
        trsc_rate = riboTranscriptRate(
            rnaMetID, newMetID, rnasequence, jcvi2ID)
        RiboPtnTrscRates.append(trsc_rate)

        translat_rate = TranslatRate(
            rnaMetID, newMetID, rnasequence, aasequence, specDict)
        ...
        sim.addReaction(geneMetID, tuple(TrscProd), riboTranscriptRate(
            rnaMetID, newMetID, rnasequence, jcvi2ID))
        sim.addReaction(rnaMetID, tuple(mRNAdegProd),
                        DegradationRate(rnaMetID, rnasequence))
        sim.addReaction(rnaMetID, tuple(TranslatProd), TranslatRate(
    ...
    def addtRNA(unchargedMetID, chargedMetID, mmcode, aminoAcid, synthase):
        ...
        if unchargedMetID not in tRNAadded:
            ...
            if 'GLN' not in synthase:
                ...
                sim.addReaction(('M_atp_c', synthase_ptn),
                                (synthase_atp), 0.01)
                sim.addReaction((aminoAcid, synthase_atp),
                                (synthase_atp_aa), 0.01)
                sim.addReaction((synthase_atp_aa, unchargedMetID),
                                (synthase_atp_aa_trna), 0.1)
                sim.addReaction(
                    (synthase_atp_aa_trna), (chargedMetID, 'M_amp_c', 'M_ppi_c', synthase_ptn), 30)

                costID = ctRNAcostMap[chargedMetID]

                cost_paid = cost + '_paid'

                sim.addReaction((chargedMetID, costID),
                                (unchargedMetID, cost_paid), 100)
                ...
                sim.addReaction((synthase_atp_aa, 'M_trnagln_c'),
                                (synthase_atp_aa_trna), 0.1)
                sim.addReaction((synthase_atp_aa_trna), ('M_glutrnagln_c',
                                'M_amp_c', 'M_ppi_c', synthase_ptn), 30)
                ...
                sim.addReaction(
                    (glugln, 'M_PTN_JCVISYN3A_0689_c'), (glugln_enz), 0.1)
                sim.addReaction((glugln_enz, 'M_atp_c'),
                                (glugln_enz_atp), 0.01)
                sim.addReaction((glugln_enz_atp, 'M_gln__L_c'),
                                (glugln_enz_atp_gln), 0.01)
                sim.addReaction((glugln_enz_atp_gln), ('M_glntrna_c', 'M_glu__L_c',
                                'M_adp_c', 'M_pi_c', 'M_PTN_JCVISYN3A_0689_c'), 30)
                sim.addReaction((glugln_enz_atp, 'M_asn__L_c'),
                                (glugln_enz_atp_asn), 0.001)
                sim.addReaction((glugln_enz_atp_asn), ('M_glntrna_c', 'M_asp__L_c',
                                'M_adp_c', 'M_pi_c', 'M_PTN_JCVISYN3A_0689_c'), 30)
                ...
                sim.addReaction(('M_glntrna_c', costID),
                                ('M_trnagln_c', cost_paid), 100)
        ...
        sim.addReaction(geneMetID, tuple(TrscProd),
                        trnaTranscriptRate(unchargedMetID, rnasequence))
    ...
    def addrRNA():
        ...
        rrna_pos_1 = Seq(
            str(genome3A.seq[min(rrna_gene_locs_1):max(rrna_gene_locs_1)]))
        ...
        rrna_pos_2 = Seq(
            str(genome3A.seq[min(rrna_gene_locs_2):max(rrna_gene_locs_2)]))
        ...
        TrscProd = ['JCVISYN3A_0067_gene', 'M_rRNA_5S_c',
                    'M_rRNA_23S_c', 'M_rRNA_16S_c']
        ...
        sim.addReaction('JCVISYN3A_0067_gene', tuple(
            TrscProd), rrnaTranscriptRate(rnasequence))
        ...
        TrscProd = ['JCVISYN3A_0532_gene', 'M_rRNA_5S_c',
                    'M_rRNA_23S_c', 'M_rRNA_16S_c']
        ...
        sim.addReaction('JCVISYN3A_0532_gene', tuple(
            TrscProd), rrnaTranscriptRate(rnasequence))
    ...
    for index, row in PtnMetDF.iterrows():
        addNamedPtn(row["species"], row["gene"],
                    row["transcribe"], row["proteomics_fraction"])

    for index, row in memPtnMetDF.iterrows():
        addMembranePtn(row["species"], row["gene"],
                       row["transcribe"], row["proteomics_fraction"])
    ...
    for index, row in trnaMetDF.iterrows():
        addtRNA(row["uncharged"], row["charged"],
                row["gene"], row["AA"], row["synthase"])
    ...
    fn = '../simulations/out-' + str(procid) + 'rs' + str(iteration) + '.lm'
    my_log_file = '../simulations/log-' + \
        str(procid) + 'rs' + str(iteration) + '.log'
    ...
    with open(my_log_file, 'w') as f:  # , redirect_stdout(f):
        odeHookSolver = hook.MyOwnSolver(
            delt, odestep, mySpecies, cythonBool, resTime, str(procid), iteration)

        sim.runSolver(filename=fn, solver=odeHookSolver,
                      replicates=1, cudaDevices=None)
