...
species_list = ['M_atp_c', 'M_adp_c', 'M_amp_c', 'M_pep_c', 'M_pyr_c', 'CellSA',
                'CELLSA_Lip', 'CellSA_Prot', 'M_pyr_e']  # Add more species as needed
...
for i, species in enumerate(species_list):
    try:
        ...
        conc = ta[:, specInd, startTime:endTime+1]*1000.0 / \
            (ta[:, vInd, startTime:endTime+1]*1e-19*6.022e23)

        # Cycle through colors if more species than colors
        color = colors[i % len(colors)]

        # Plot median line for each species
        plt.plot(times[:], np.median(conc, axis=0),
                 label=f'{species} (median)', color=color, linewidth=2)
...
plt.title(
    f'Time Course of Multiple Metabolites ({len(species_list)} species)', fontsize=16)
...
def sdMinus(conc):
    arr = np.maximum(np.average(conc, axis=0) -
                     np.std(conc, axis=0), zeros[0, 0, :])
    return arr


# %%
# Plot multiple chemical species in number of particles
species_particles_list = ['M_DnaA_c', 'M_PTN_JCVISYN3A_0001_c',
                          'M_PTN_JCVISYN3A_0002_c', 'CellV']  # Add more species as needed
times = np.arange(startTime, endTime+1, 1)
...
for i, species in enumerate(species_particles_list):
    try:
        ...
        plt.plot(times[:], np.average(parts, axis=0),
                 label=f'{species} (avg)', color=color, linewidth=2)
...
plt.legend(fontsize=12, loc='best')
plt.title(
    f'Time Course of Multiple Species ({len(species_particles_list)} species)', fontsize=16)
plt.grid(True, alpha=0.3)
# %%
...
plt.fill_between(times[:], np.percentile(flux, 1, axis=0), np.percentile(
    flux, 99, axis=0), alpha=0.3, label='1-99%', color='black')
plt.fill_between(times[:], np.average(flux, axis=0)+np.std(flux, axis=0), np.average(
    flux, axis=0)-np.std(flux, axis=0), alpha=0.5, label='Stdev', color='magenta')
