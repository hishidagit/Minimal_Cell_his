...
# nohup python3 run_simulation.py -procid $id -t $ti > output.log 2>&1 &
...
import lm as lm
import sys
import hook_start_timecourse as hook
import time
...
# args = ap.parse_args()
# set procid and time manually if no arguments are given
...
proteomPD = pd.read_excel("../model_data/proteomics.xlsx",
...
ModelSpecies = []
defaultPtnCount = 10
baseMap = OrderedDict({"A": "M_atp_c", "U": "M_utp_c",
                      "G": "M_gtp_c", "C": "M_ctp_c"})
...
def addPtn(sim, jcvi3AID):
    ...
    if (rnasequence != 0) and (aasequence != 0):
        ...
        sim.addReaction(rnaMetID, tuple(TranslatProd), TranslatRate(
            rnaMetID, ptnMetID, rnasequence, aasequence))


def addNamedPtn(sim, newMetID, mmcode, transcribe, ptnFrac):
    ...
    if transcribe:
        ...
        sim.addReaction(rnaMetID, tuple(mRNAdegProd),
                        DegradationRate(rnaMetID, rnasequence))
        sim.addReaction(rnaMetID, tuple(TranslatProd), TranslatRate(
            rnaMetID, newMetID, rnasequence, aasequence))
...
def addMembranePtn(sim, newMetID, mmcode, transcribe, ptnFrac):
...
def addRiboPtn(sim, newMetID, mmcode):
...
def addtRNA(sim, unchargedMetID, chargedMetID, mmcode, aminoAcid, synthase):
    ...
    if unchargedMetID not in tRNAadded:
        ...
        if 'GLN' not in synthase:
            ...
            sim.addReaction((synthase_atp_aa_trna), (chargedMetID,
            ...
            sim.addReaction((glugln, 'M_PTN_JCVISYN3A_0689_c'),
                            (glugln_enz), 0.01)
...
for index, row in PtnMetDF.iterrows():
    addNamedPtn(sim, row["species"], row["gene"],
                row["transcribe"], row["proteomics_fraction"])

for index, row in memPtnMetDF.iterrows():
    addMembranePtn(sim, row["species"], row["gene"],
                   row["transcribe"], row["proteomics_fraction"])

for index, row in riboPtnMetDF.iterrows():
    addRiboPtn(sim, row["species"], row["gene"])
...
for index, row in trnaMetDF.iterrows():
    addtRNA(sim, row["uncharged"], row["charged"],
...
# Constants
delt = 1.0  # s - hook interval (time-course sampling)
odestep = delt/10.0  # 100.0 # s
# s - write interval (changed to match hook for time-course output)
write = 1.0
simTime = st*60.0  # s
...
cythonBool = True  # For smaller system no cython (scipy.ode)
...
with open(my_log_file, 'w') as f:  # , redirect_stdout(f):
    odeHookSolver = hook.MyOwnSolver(
        delt, odestep, mySpecies, cythonBool, totalTime, str(procid))
